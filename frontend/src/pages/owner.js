...
const owner = () => {
  ...
  const getPets = async () => {
    try {
      const res = await axios.get("https://furrycareserver.onrender.com/pets", {
        headers: {
  ...
  const getVets = async () => {
    try {
      const res = await axios.get("https://furrycareserver.onrender.com/vets", {
        headers: {
          Authorization: `Bearer ${data.token}`,
        },
      });
  ...
  const getCaretakers = async () => {
    try {
      const res = await axios.get(
        "https://furrycareserver.onrender.com/caretakers",
        {
          headers: {
            Authorization: `Bearer ${data.token}`,
          },
